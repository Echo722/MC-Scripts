...
--
-- @changelog
--   * Pin On Top now stays topmost only while REAPER is the foreground app, so
--     the window no longer covers other programs (e.g. a browser) when you
--     switch away; it re-pins automatically when REAPER regains focus.
--   ----
--   * Pin On Top(置顶)改为仅在 REAPER 处于前台时生效,切到其它程序(如浏览器)
--     时不再遮挡,切回 REAPER 时自动重新置顶。
...
local topmost        = (reaper.GetExtState(EXT_SECTION, "topmost") == "1")
local info_btn_rect  = { x = 0, y = 0, w = 0, h = 0 }
local pin_btn_rect   = { x = 0, y = 0, w = 0, h = 0 }
...
-- Effective Z-order has three targets, re-evaluated (throttled) each frame and only
-- written when it changes:
--   "top"  = pref on & REAPER is foreground     -> TOPMOST (pinned above REAPER windows)
--   "away" = pref on & another app is foreground -> BOTTOM. NOT "NOTOPMOST": NOTOPMOST
--            re-inserts the window at the top of the non-topmost band, which drags
--            MediaEye's owner (the REAPER main window) up above the app you just
--            clicked (that window then flashes and drops behind REAPER). BOTTOM also
--            clears the topmost flag but pulls the owner group down, so the clicked
--            window stays on top.
--   "off"  = pref off                            -> NOTOPMOST (behave as a normal window)
local applied_z   = nil   -- "top" | "away" | "off" | nil (unknown)
local last_z_time = 0
...
local function update_topmost(force)
    ...
    if not topmost then
        target = "off"
    elseif reaper_is_foreground() then
        target = "top"
    else
        target = "away"
    end
    if target == applied_z then return end

    local hwnd = refresh_script_hwnd()
    if not hwnd or not reaper.JS_Window_SetZOrder then return end

    local zorder
    if target == "top" then zorder = "TOPMOST"
    elseif target == "away" then zorder = "BOTTOM"
    else zorder = "NOTOPMOST" end

    if reaper.JS_Window_SetZOrder(hwnd, zorder) then
        applied_z = target
    end
end
...
local function handle_mouse()
    ...
    if pressed_now then
        ...
        elseif point_in_rect(gfx.mouse_x, gfx.mouse_y, pin_btn_rect) then
            topmost = not topmost
            reaper.SetExtState(EXT_SECTION, "topmost", topmost and "1" or "0", true)
            update_topmost(true)
        end
    end
    last_mouse_cap = cap
...
local function draw_controls()
    local pin_label  = topmost and "Pinned" or "Pin On Top"
    ...
    local pin_tw = select(1, gfx.measurestr(pin_label))
    local pin_w, pin_h = pin_tw + 16, select(2, gfx.measurestr(pin_label)) + 8
    draw_button(pin_label, pin_btn_rect, gfx.w - pin_w - 6, gfx.h - pin_h - 6)
end
