...
--
-- @changelog
--   * The window now stays above the Media Explorer only, instead of above every
--     REAPER window: clicking an FX (or any other) window brings it over the
--     cover art as usual, so large plugin UIs are no longer obstructed. The
--     above-ME ordering is maintained natively by Windows (owner attachment),
--     so clicking around the Media Explorer causes no flicker. Hiding when
--     another application is in the foreground is unchanged.
--   ----
--   * 窗口置顶范围收窄:只保持在 Media Explorer 之上,不再压住所有 REAPER 窗口——
--     点击 FX 等其它窗口可正常盖住封面图,大插件界面不再被挡;在 ME 上层的排序
--     由 Windows 原生维持(owner 挂接),点击 ME 无闪烁;切到其它程序时自动隐藏
--     的行为保持不变。
...
local function find_folder_image(filepath)
    ...
    local best_path, best_score, best_named = nil, 0, false
    ...
    local function scan_dir(dir, weight)
        ...
        while true do
            ...
            if IMG_EXTS[ext] then
                ...
                local raw   = score_name(stem)
                local score = raw * weight
                if score > best_score then
                    best_score = score
                    best_path  = dir .. "/" .. fname
                    best_named = raw >= 3   -- stem carries a cover keyword
                end
            end
        end
    end
    ...
    -- Second value: whether the winner is *named* as a cover (keyword match),
    -- as opposed to merely being some image found nearby.
    return best_path, best_named
end
...
-- frame:
--   "top"  = REAPER is the foreground app -> window shown, and kept directly above
--            the Media Explorer ONLY. No WS_EX_TOPMOST anywhere: being an owned
--            window already keeps it above the main window, and other REAPER
--            windows (FX etc.) may legitimately cover it when clicked — the cover
--            art only ever needs to beat the Media Explorer.
--   "away" = another app is foreground    -> window hidden. Hiding (instead of
--            demoting) sidesteps a Win11 bug where no demotion reliably clears
--            WS_EX_TOPMOST; hidden, the window keeps its z-position, so showing
--            it again fully restores it.
...
local fg_is_reaper = false -- cached verdict from the last throttled tick
local last_z_time  = 0
local Z_INTERVAL   = 0.2

-- Owning the script window to the floating Media Explorer makes Windows itself
-- keep it above the ME through every activation: clicking the ME raises the two
-- together, so there is no fall-behind-then-correct blink at all. Other REAPER
-- windows (FX etc.) are outside this pair and can still cover us. If the ME
-- closes, its DestroyWindow cascade destroys owned windows too — main()
-- resurrects the gfx window when that happens.
local owned_to_me = false

local function update_owner(hwnd)
    if not (reaper.JS_Window_SetLong and reaper.JS_Window_GetRelated
            and reaper.JS_Window_AddressFromHandle) then
        return
    end
    local main_hwnd  = reaper.GetMainHwnd()
    local cur_owner  = reaper.JS_Window_GetRelated(hwnd, "OWNER")
    if is_valid(me_hwnd) and reaper.JS_Window_GetRelated(me_hwnd, "OWNER") == main_hwnd then
        if cur_owner ~= me_hwnd then
            reaper.JS_Window_SetLong(hwnd, "PARENT", reaper.JS_Window_AddressFromHandle(me_hwnd))
        end
        owned_to_me = true
    elseif owned_to_me then
        -- ME gone or docked: fall back to the main window as owner so we never
        -- keep a stale owner handle.
        if cur_owner ~= main_hwnd then
            reaper.JS_Window_SetLong(hwnd, "PARENT", reaper.JS_Window_AddressFromHandle(main_hwnd))
        end
        owned_to_me = false
    end
end

-- Keep the script window directly above the (floating) Media Explorer in the
-- normal z-band. With the owner attachment above this is belt-and-braces (the
-- OS maintains the order); it still matters right after (re)attaching, and as
-- the fallback when the owner APIs are missing. Writes only when MediaEye has
-- actually fallen below the ME, so steady state is read-only. Never called
-- unless REAPER is the foreground app, so raising here cannot drag REAPER's
-- owner group above another application.
local function ensure_above_me(hwnd)
    if not is_valid(me_hwnd) or not reaper.JS_Window_GetRelated
       or not reaper.JS_Window_SetZOrder then
        return
    end
    -- Docked ME lives inside the main window; owned-above-owner already covers it.
    local owner = reaper.JS_Window_GetRelated(me_hwnd, "OWNER")
    if owner ~= reaper.GetMainHwnd() then return end

    -- Walk upward (PREV = window above) from the ME: if we meet ourselves, we
    -- are already above it — the common case, no write.
    local w, guard = me_hwnd, 0
    while guard < 40 do
        w = reaper.JS_Window_GetRelated(w, "PREV")
        if not w then break end
        if w == hwnd then return end
        guard = guard + 1
    end

    local above = reaper.JS_Window_GetRelated(me_hwnd, "PREV")
    if above == hwnd then return end  -- unreachable after the walk; defensive
    if not above then
        reaper.JS_Window_SetZOrder(hwnd, "TOP")
    elseif get_ex_topmost(above) == 1 then
        -- The window right above the ME is in the topmost band; inserting after
        -- it would drag us into that band. Top of the normal band is enough.
        reaper.JS_Window_SetZOrder(hwnd, "TOP")
    else
        -- Insert below `above` = directly above the ME.
        reaper.JS_Window_SetZOrder(hwnd, "INSERTAFTER", above)
    end
end

local function update_topmost(force)
    -- Throttled part: foreground detection + show/hide state machine. Tick-based
    -- debounce (away_streak) relies on this interval, so it stays at Z_INTERVAL.
    local now = reaper.time_precise()
    if force or (now - last_z_time) >= Z_INTERVAL then
        last_z_time = now
        ...
        if gfx.dock(-1) % 2 == 1 then
            ...
            applied_z, away_streak, fg_is_reaper, owned_to_me = nil, 0, false, false
            return
        end

        local fg_ok = reaper_is_foreground()
        if fg_ok ~= nil then fg_is_reaper = (fg_ok == true) end  -- shell: keep last verdict
    ...
    -- Per-frame part: keep the owner attachment current (the OS then maintains
    -- us above the ME natively — zero flicker), plus the read-only z check as
    -- fallback. Writes only on attach/detach or when actually displaced.
    if applied_z == "top" and fg_is_reaper and gfx.dock(-1) % 2 == 0 then
        local hwnd = refresh_script_hwnd()
        if hwnd then
            update_owner(hwnd)
            ensure_above_me(hwnd)
        end
    end
end
...
local function process_selection()
    ...
    -- Audio metadata + embedded picture bytes (picture loaded into a slot later,
    -- only if it actually wins the priority race below).
    local embedded = nil
    ...
    last_pic_size = (embedded and embedded.picture) and #embedded.picture or 0

    -- Cover priority: an image explicitly NAMED as cover art (Artwork/Cover/
    -- Folder/Front/...) on the folder chain beats the embedded picture — some
    -- vendors (e.g. SwishSwoosh) embed their label logo into every file while
    -- the real pack cover sits in the root as Artwork.png. The embedded picture
    -- still beats images that merely happen to be nearby.
    local folder_path, folder_named = find_folder_image(path)

    if folder_named and folder_path then
        entry.img_slot = load_image_from_file(folder_path)
        if entry.img_slot then entry.source = "folder-image (named)" end
    end

    if not entry.img_slot and embedded and embedded.picture then
        entry.img_slot = load_image_from_bytes(embedded.picture, embedded.mime)
        entry.source   = entry.img_slot
            and string.format("embedded %s %dB", embedded.mime or "?", #embedded.picture)
            or  string.format("embed-decode-failed %s %dB", embedded.mime or "?", #embedded.picture)
    end

    if not entry.img_slot and folder_path and not folder_named then
        entry.img_slot = load_image_from_file(folder_path)
        if entry.img_slot then entry.source = "folder-image" end
    end

    if not entry.source then entry.source = "no cover found" end
...
--------------------------------------------------------------------------------
-- Captured every frame while the window is alive: when the window dies (user
-- close, or destroyed by the owner ME closing) its geometry is no longer
-- queryable, so both saving and resurrection read this snapshot instead.
local last_wstate = nil

local function capture_window_state()
    local dock, x, y = gfx.dock(-1, 0, 0, 0, 0)
    last_wstate = { w = gfx.w, h = gfx.h, x = x, y = y, dock = dock }
end
...
local function save_window_state()
    ...
    if not s then
        local dock, x, y = gfx.dock(-1, 0, 0, 0, 0)
        s = { w = gfx.w, h = gfx.h, x = x, y = y, dock = dock }
    end
    reaper.SetExtState(EXT_SECTION, "w",    tostring(s.w),    true)
    reaper.SetExtState(EXT_SECTION, "h",    tostring(s.h),    true)
    reaper.SetExtState(EXT_SECTION, "x",    tostring(s.x),    true)
    reaper.SetExtState(EXT_SECTION, "y",    tostring(s.y),    true)
    reaper.SetExtState(EXT_SECTION, "dock", tostring(s.dock), true)
end
...
local function main()
  ...
  handle_mouse()
    ...
    if gfx.getchar() >= 0 then
        ...
        -- Window is gone. If we were owned to the floating Media Explorer and
        -- the ME vanished too, its DestroyWindow cascade took us with it — the
        -- user closed the ME, not MediaEye. Bring the window back.
        if owned_to_me and not is_valid(me_hwnd) then
            owned_to_me  = false
            script_hwnd  = nil
            applied_z, away_streak, fg_is_reaper = nil, 0, false
            local s = last_wstate
            gfx.quit()
            gfx.init(WINDOW_TITLE, s and s.w or 400, s and s.h or 460,
                     s and s.dock or 0, s and s.x or 200, s and s.y or 200)
