...
--
-- @changelog
--   * Move the ReaPack install path to MC Scripts Release/MediaEye.
--   * Recognise "Doc" as an asset folder name (alongside Documents / Docs /
--     Artwork / Pictures / ...) so libraries packaged with a top-level
--     Doc/ folder also surface their cover art.
--   ----
--   * 封面资源文件夹识别新增 "Doc" 命名(与 Documents / Docs / Artwork /
--     Pictures 等同等待遇),让把封面放在 Doc/ 目录里的库也能正确显示。
...
local function apply_topmost()
    local hwnd = refresh_script_hwnd()
    if not hwnd or not reaper.JS_Window_SetZOrder then return false end
    return reaper.JS_Window_SetZOrder(hwnd, topmost and "TOPMOST" or "NOTOPMOST")
end
...
-- Main loop
--------------------------------------------------------------------------------
local topmost_applied = false

local function main()
    if not topmost_applied then
        topmost_applied = apply_topmost()
    end
