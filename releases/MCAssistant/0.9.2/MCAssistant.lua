...
-- @changelog
--   * 0.9.2  Fix laggy / duplicated / dropped text paste in the input box.
--            Ctrl+V used to synchronously spawn PowerShell (Get-Clipboard
--            -Format Image) on EVERY paste to check for an image, freezing the
--            UI thread for the powershell cold-start. That stall let the held
--            Ctrl+V auto-repeat, so ImGui's native text paste fired twice (and
--            sometimes dropped). Now the image grab only runs when the
--            clipboard has no text (checked via reaper.ImGui_GetClipboardText);
--            text pastes go straight through ImGui with no process spawn.
--            ui.lua only.
