...
local function summarize_tool_output(output)
    ...
    if output.purpose or output.api_calls or output.returned ~= nil or output.prints then
        if output.ok == false then
            local msg = tostring(output.error or "?")
            if output.prints and output.prints ~= "" then
                msg = msg .. "  prints=" .. tostring(output.prints):sub(1, 80)
            end
            return "error=" .. msg
        end
        local exec_hints = {}
        if output.api_calls ~= nil then exec_hints[#exec_hints + 1] = "api_calls=" .. tostring(output.api_calls) end
        if output.returned ~= nil then
            local v = output.returned
            if type(v) == "table" then v = "[table]" end
            exec_hints[#exec_hints + 1] = "returned=" .. tostring(v):sub(1, 60)
        end
        if output.prints and output.prints ~= "" then
            exec_hints[#exec_hints + 1] = "prints=" .. tostring(output.prints):sub(1, 80)
        end
        if #exec_hints > 0 then return table.concat(exec_hints, "  ") end
    end
...
function M.new(chat, opts)
    ...
    return {
        ...
        on_update_confirm        = opts.on_update_confirm,
        on_update_dismiss        = opts.on_update_dismiss,

        -- Update-available dialog state (raised by the startup check)
        _open_update_request     = false,
        _update_new_version      = "",
        _update_current_version  = "",

        input_buf                = "",
        _input_capture_cb        = nil,
        _input_seq               = 0,      -- bumped on clear to force a fresh InputText ID
        _refocus_after_clear     = false,  -- re-focus the input the frame after a send
...
-- Clear the InputText buffer. Resetting the Lua-side string alone is not
-- enough: ReaImGui keeps the widget's buffer keyed by its ImGui ID and
-- redisplays it, so the just-sent text reappears once the input renders
-- again after the AI finishes. Two things actually clear it:
--   1. Bump _input_seq so the InputText gets a fresh ImGui ID next frame —
--      ReaImGui then re-seeds the new widget from our empty string.
--   2. Wipe the EEL capture slot (#LastBuf). The capture callback only fires
--      while the widget is rendered + active, so during AI generation (busy
--      branch skips the widget entirely) #LastBuf goes stale at the last
--      non-empty value. Without this, the stale value is read back on the
--      first idle frame and resurrects the sent text even with a fresh ID.
-- (The busy branch skipping InputTextMultiline is a separate fix — it avoids
-- the EEL CallbackAlways + ReadOnly combo that BSOD'd the GPU driver.)
local function clear_input_buf(ui)
    ui.input_buf = ""
    ui._input_seq = (ui._input_seq or 0) + 1
    ui._refocus_after_clear = true
    if ui._input_capture_cb and reaper.ImGui_Function_SetValue_String then
        reaper.ImGui_Function_SetValue_String(ui._input_capture_cb, "#LastBuf", "")
    end
end
...
local function draw_input_stable(ctx, ui)
    ...
    if not is_busy then
        ...
        reaper.ImGui_PushStyleVar(ctx, reaper.ImGui_StyleVar_FrameRounding(), 0)
        reaper.ImGui_PushStyleVar(ctx, reaper.ImGui_StyleVar_FramePadding(), 0, 4)
        -- Re-focus the input on the first idle frame after a send so the user
        -- can keep typing without clicking back in.
        if ui._refocus_after_clear then
            if reaper.ImGui_SetKeyboardFocusHere then
                reaper.ImGui_SetKeyboardFocusHere(ctx)
            end
            ui._refocus_after_clear = false
        end
        local prev_buf = ui.input_buf or ""
        -- _input_seq is part of the ID so clear_input_buf can force ReaImGui to
        -- drop the old buffer (see that function's comment).
        local rv, new_buf = reaper.ImGui_InputTextMultiline(ctx,
            "##input_main_" .. (ui._input_seq or 0),
...
--------------------------------------------------------------------------------
local UPDATE_POPUP = "MCAssistant 更新##mcup"

-- Raised by the startup update check (MCAssistant.lua sets _open_update_request
-- + the version fields). "现在更新" → on_update_confirm (opens ReaPack browser);
-- "以后再说" → on_update_dismiss (remembers the version so it won't nag again).
local function draw_update_popup(ctx, ui)
    if ui._open_update_request then
        reaper.ImGui_OpenPopup(ctx, UPDATE_POPUP)
        ui._open_update_request = false
    end
    ...
    reaper.ImGui_PushStyleVar(ctx, reaper.ImGui_StyleVar_WindowRounding(),   8)
    reaper.ImGui_PushStyleVar(ctx, reaper.ImGui_StyleVar_FrameRounding(),    6)
    reaper.ImGui_PushStyleVar(ctx, reaper.ImGui_StyleVar_WindowPadding(),    18, 18)
    ...
    if not reaper.ImGui_BeginPopupModal(ctx, UPDATE_POPUP, nil, flags) then
        reaper.ImGui_PopStyleVar(ctx, 4)
        reaper.ImGui_PopStyleColor(ctx, 2)
        return
    end
    ...
    reaper.ImGui_PushFont(ctx, ui.font, 18)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), COL.title)
    reaper.ImGui_Text(ctx, "发现新版本")
    reaper.ImGui_PopStyleColor(ctx)
    reaper.ImGui_PopFont(ctx)

    reaper.ImGui_Dummy(ctx, 1, 8)
    reaper.ImGui_Text(ctx, ("MCAssistant v%s 可用。"):format(ui._update_new_version or ""))
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), COL.very_dim)
    reaper.ImGui_Text(ctx, ("当前版本 v%s"):format(ui._update_current_version or ""))
    reaper.ImGui_PopStyleColor(ctx)
    reaper.ImGui_Dummy(ctx, 1, 14)

    local avail = reaper.ImGui_GetContentRegionAvail(ctx)
    local footer_w = 110 + 120 + 12
    reaper.ImGui_Dummy(ctx, math.max(1, avail - footer_w), 1)
    reaper.ImGui_SameLine(ctx)
    if outline_button(ctx, "以后再说", 110, 36)
        or reaper.ImGui_IsKeyPressed(ctx, reaper.ImGui_Key_Escape(), false) then
        if ui.on_update_dismiss then ui.on_update_dismiss(ui._update_new_version) end
        reaper.ImGui_CloseCurrentPopup(ctx)
    end
    reaper.ImGui_SameLine(ctx)
    if filled_button(ctx, "现在更新", 120, 36) then
        if ui.on_update_confirm then ui.on_update_confirm() end
        reaper.ImGui_CloseCurrentPopup(ctx)
    end
...
local function handle_shortcuts(ctx, ui)
    -- Ctrl+V: paste an IMAGE from the clipboard (only when idle).
    -- ImGui's InputText already pastes TEXT natively. The image grab
    -- (image.paste_clipboard) synchronously spawns PowerShell (Get-Clipboard
    -- -Format Image), which freezes the UI thread for the powershell
    -- cold-start. Running it on EVERY Ctrl+V — including text pastes — stalled
    -- the frame long enough that the held Ctrl+V auto-repeated and ImGui's
    -- native paste fired twice (duplicate text), and sometimes dropped.
    -- Fix: if the clipboard already holds text, this is a text paste — let
    -- ImGui handle it and skip the image path entirely. Only spawn PowerShell
    -- when there's no text (i.e. the clipboard likely holds an image).
    if ui.chat.state == "idle" then
        local ctrl_v = reaper.ImGui_Mod_Ctrl() | reaper.ImGui_Key_V()
        if reaper.ImGui_IsKeyChordPressed(ctx, ctrl_v) then
            local clip_text = reaper.ImGui_GetClipboardText
                and reaper.ImGui_GetClipboardText(ctx) or nil
            if not (clip_text and clip_text ~= "") then
                if not image_mod then image_mod = require("image") end
