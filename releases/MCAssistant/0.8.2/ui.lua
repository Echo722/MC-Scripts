...
local function draw_input(ctx, ui)
    ...
    local clip_btn_w = 28
    local btn_w   = 80
    local input_w = math.max(120, avail_w - clip_btn_w - btn_w - 12)
    ...
    reaper.ImGui_SameLine(ctx)
    if is_busy then
        reaper.ImGui_Dummy(ctx, clip_btn_w, input_h)
    else
        reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), COL.dim)
        if reaper.ImGui_Button(ctx, "📎", clip_btn_w, input_h) then
