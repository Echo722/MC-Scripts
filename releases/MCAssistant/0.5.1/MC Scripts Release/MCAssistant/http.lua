...
local function build_curl_cmd(url, headers, body_path, resp_path, hdr_path, stream)
    local parts = { "call", q(CURL), "-sS", "-X", "POST" }
    if stream then
        parts[#parts + 1] = "-N"
...
function M.start(url, headers, body, opts)
 ...
 local dir = ensure_temp_dir()
    local id = unique_id()
    local body_path = to_win_path(dir .. "\\req_"  .. id .. ".bin")
 local resp_path = to_win_path(dir .. "\\resp_" .. id .. ".bin")
    ...
    if not write_text(body_path, body or "") then
 return { id = id, done = true, err = "failed to write req body" }
    end
    write_text(resp_path, "")
    write_text(hdr_path, "")

  local curl_cmd = build_curl_cmd(url, headers, body_path, resp_path, hdr_path, opts.stream)
