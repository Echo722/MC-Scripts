...
--     - claude — Anthropic Messages API (api.anthropic.com or proxy)
...
常见预设 (type / base URL / model / api key):
  Anthropic:  claude / (留空) / claude-sonnet-4-6 / sk-ant-...
...
local function prompt_settings(current, show_help)
    ...
    local fields = "Type (claude/openai),Base URL (blank=default),Model,API key,extrawidth=520,separator=|"
    local default = table.concat({
   (current and current.type)     or "claude",
...
base_url = trim(parts[2]),
   model    = trim(parts[3]),
    ...
    if s.type ~= "claude" and s.type ~= "openai" then
        reaper.MB("Provider type 必须是 'claude' 或 'openai'。", "MCAssistant", 0)
        return nil
    end
    if s.api_key == "" then
