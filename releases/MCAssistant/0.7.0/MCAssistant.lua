...
local function refresh_overlay()
    ...
    ui_mod.open_settings_overlay(ui,
        chat.provider.name, chat.provider.model,
        settings.search_api_key or "")
end
...
local function on_overlay_edit_key()
    local k = prompt_search_key(settings.search_api_key or "")
    if k and k ~= "" then
        settings.search_api_key = k
        save_settings(settings)
    end
    refresh_overlay()
end
...
local function on_toggle_search(_is_right)
    ...
    if new_on and (settings.search_api_key or "") == "" then
        local k = prompt_search_key("")
        if k and k ~= "" then
            settings.search_api_key = k
            save_settings(settings)
        end
...
ui = ui_mod.new(chat, {
    ...
    on_overlay_edit_provider = on_overlay_edit_provider,
    on_overlay_edit_key      = on_overlay_edit_key,
    hwnd                     = hwnd,
})
