...
local VERSION = read_own_version(SELF_PATH) or "0.8.5"
...
-- Update-dialog callbacks. "现在更新" opens the ReaPack browser pre-filtered
-- to MCAssistant (guarded — ReaPack API may be absent); "以后再说" remembers
-- the version so we don't prompt for it again.
...
local function on_update_dismiss(v)
    reaper.SetExtState(EXT, "skip_update_version", v or "", true)
end
...
ui = ui_mod.new(chat, {
    ...
    on_update_dismiss = on_update_dismiss,
})

ui_mod.set_web_search_state(ui, is_web_search_enabled_for(settings))

-- Fire the one-shot startup update check (async; polled in main()).
local update_handle = update and update.start() or nil
...
local function main()
    ...
    -- Poll the startup update check (one-shot). When a newer version is
    -- available and the user hasn't dismissed it, raise the confirm dialog.
    if update_handle then
        local r = update.poll(update_handle)
        if r.done then
            local skip = reaper.GetExtState(EXT, "skip_update_version")
            if r.latest and update.compare(VERSION, r.latest) < 0 and r.latest ~= skip then
