...
-- event rendering
--
-- Static events (user / tool_call / tool_result / error) get a rounded bubble
-- via a FIXED-SIZE BeginChild — height is pre-measured with CalcTextSize so we
-- never set AutoResize flags. The earlier ChildFlags_AlwaysAutoResize approach
-- crashed during streaming because the child kept re-measuring; fixed-size
-- works because the content is immutable after the event is appended.
--
-- The streaming `assistant` event has variable height — it stays as a plain
-- TextWrapped (no bubble), which matches the Claude-desktop look anyway.
--------------------------------------------------------------------------------
...
-- Render text inside a rounded bubble using the window draw list. Order:
--   1. Compute bubble size from CalcTextSize (with the actual wrap width).
--   2. Add the filled rect to the draw list FIRST → it renders behind text.
--   3. Move cursor inside the bubble's content area, render text on top.
--   4. Reserve the bubble's full size in the layout via a final Dummy so
--      auto-scroll / content sizing accounts for it.
--
-- This replaces an earlier BeginChild-based bubble: with WindowPadding
-- pushed onto child windows, text was rendering outside the child's bounds
-- on this ReaImGui build. DrawList gives us explicit control.
local function draw_bubble(ctx, text, max_w, align_right, bg_color, text_color, rounding)
    local pad = 12
    local content_w = math.max(40, max_w - pad * 2)
    local tw, _   = reaper.ImGui_CalcTextSize(ctx, text, content_w)
    local bubble_w = math.min(math.ceil(tw) + pad * 2 + 4, max_w)
    local inner_w  = bubble_w - pad * 2
    local _, th    = reaper.ImGui_CalcTextSize(ctx, text, inner_w)
    local bubble_h = math.ceil(th) + pad * 2

    if align_right then
        local avail_w = reaper.ImGui_GetContentRegionAvail(ctx)
        local x_off = avail_w - bubble_w
        if x_off > 0 then
            reaper.ImGui_SetCursorPosX(ctx,
                reaper.ImGui_GetCursorPosX(ctx) + x_off)
        end
    end

    local sx, sy = reaper.ImGui_GetCursorScreenPos(ctx)
    local draw_list = reaper.ImGui_GetWindowDrawList(ctx)
    reaper.ImGui_DrawList_AddRectFilled(draw_list, sx, sy,
        sx + bubble_w, sy + bubble_h, bg_color, rounding or 10)

    -- Render text on top of the rect, clipped to the bubble interior.
    reaper.ImGui_SetCursorScreenPos(ctx, sx + pad, sy + pad)
    reaper.ImGui_PushStyleColor(ctx, reaper.ImGui_Col_Text(), text_color)
    reaper.ImGui_PushTextWrapPos(ctx, sx + bubble_w - pad)
    reaper.ImGui_Text(ctx, text)
    reaper.ImGui_PopTextWrapPos(ctx)
    reaper.ImGui_PopStyleColor(ctx)

    -- Reserve the full bubble extent in the layout so scrolling / spacing
    -- accounts for it (text alone may not fill the padded height).
    reaper.ImGui_SetCursorScreenPos(ctx, sx, sy)
    reaper.ImGui_Dummy(ctx, bubble_w, bubble_h)
end

local function draw_event(ctx, ev)
    local k = ev.kind

    if k == "user" then
        local avail_w = reaper.ImGui_GetContentRegionAvail(ctx)
        local max_w = math.min(math.floor(avail_w * 0.78), 560)
        draw_bubble(ctx, ev.text or "", max_w, true,
            COL.user_bubble, COL.user_text, 14)
    ...
    elseif k == "error" then
        ...
        draw_bubble(ctx, text, avail_w - 4, false,
            COL.error_bubble, COL.error_text, 6)
    end
end
...
local function draw_input(ctx, ui)
    ...
    if is_busy then
        ...
        reaper.ImGui_Text(ctx,
            "Enter 发送 · Ctrl+Enter 换行 · Ctrl+L 清屏 · Ctrl+W 关闭 · 搜索: "
